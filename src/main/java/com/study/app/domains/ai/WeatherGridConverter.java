package com.study.app.domains.ai;

/**
 * 기상청 동네예보 격자 변환 클래스
 *
 * 역할:
 * - 축제 데이터의 위도(latitude), 경도(longitude)를
 *   기상청 단기예보 API에서 사용하는 nx, ny 격자 좌표로 변환한다.
 *
 * 이유:
 * - 기상청 getVilageFcst API는 위도/경도를 직접 받지 않고
 *   nx, ny 값을 요청 인자로 사용한다.
 */
public class WeatherGridConverter {

    // 지구 반경(km)
    private static final double RE = 6371.00877;

    // 격자 간격(km)
    private static final double GRID = 5.0;

    // 투영 위도 기준값
    private static final double SLAT1 = 30.0;
    private static final double SLAT2 = 60.0;

    // 기준점 경도, 위도
    private static final double OLON = 126.0;
    private static final double OLAT = 38.0;

    // 기준점 격자 좌표
    private static final double XO = 43.0;
    private static final double YO = 136.0;

    // degree → radian 변환값
    private static final double DEGRAD = Math.PI / 180.0;

    /**
     * 위도/경도를 기상청 격자 좌표(nx, ny)로 변환
     *
     * @param longitude 경도
     * @param latitude 위도
     * @return int[] {nx, ny}
     */
    public static int[] convert(double longitude, double latitude) {

        double re = RE / GRID;

        double slat1 = SLAT1 * DEGRAD;
        double slat2 = SLAT2 * DEGRAD;
        double olon = OLON * DEGRAD;
        double olat = OLAT * DEGRAD;

        double sn = Math.tan(Math.PI * 0.25 + slat2 * 0.5)
                / Math.tan(Math.PI * 0.25 + slat1 * 0.5);

        sn = Math.log(Math.cos(slat1) / Math.cos(slat2))
                / Math.log(sn);

        double sf = Math.tan(Math.PI * 0.25 + slat1 * 0.5);
        sf = Math.pow(sf, sn) * Math.cos(slat1) / sn;

        double ro = Math.tan(Math.PI * 0.25 + olat * 0.5);
        ro = re * sf / Math.pow(ro, sn);

        double ra = Math.tan(Math.PI * 0.25 + latitude * DEGRAD * 0.5);
        ra = re * sf / Math.pow(ra, sn);

        double theta = longitude * DEGRAD - olon;

        if (theta > Math.PI) {
            theta -= 2.0 * Math.PI;
        }

        if (theta < -Math.PI) {
            theta += 2.0 * Math.PI;
        }

        theta *= sn;

        int nx = (int) Math.floor(ra * Math.sin(theta) + XO + 0.5);
        int ny = (int) Math.floor(ro - ra * Math.cos(theta) + YO + 0.5);

        return new int[] { nx, ny };
    }
}